...
# -*- coding: utf-8 -*-
"""
AOA USB Server (Android Open Accessory) + UDP (compat ESP32) + GPIO Pulses (Orange Pi / RPi / gpiod / sysfs)
...
Requisitos:
    pip install pyusb
    (opcional p/ GPIO) OrangePi.GPIO  ou RPi.GPIO  ou gpiod  (ou usa sysfs)
"""
...
GPIO_BACKEND_PRIORITY = ("OPI", "RPI", "GPIOD", "SYSFS")

# OrangePi.GPIO (pino físico)
OPI_BOARD = None
OPI_PIN   = 12
...
class GpioOPI(GpioDriverBase):
    name = "OPI(OrangePi.GPIO)"
    def __init__(self, board=None, pin=12):
        self.pin = pin
        self.board = board
        self.GPIO = None
    def setup(self):
        import importlib
        self.GPIO = importlib.import_module("OrangePi.GPIO")
        if hasattr(self.GPIO, "setboard") and self.board:
            try:
                b = getattr(self.GPIO, self.board) if isinstance(self.board, str) else self.board
                self.GPIO.setboard(b)
            except Exception:
                pass
        self.GPIO.setmode(self.GPIO.BOARD)
        self.GPIO.setwarnings(False)
        self.GPIO.setup(self.pin, self.GPIO.OUT, initial=self.GPIO.LOW)
    def pulse(self, count, on_ms, off_ms):
        if count <= 0: return
...
class GpioRPI(GpioDriverBase):
    name = "RPI(RPi.GPIO)"
    def __init__(self, bcm=18):
        self.pin = bcm
        self.GPIO = None
    def setup(self):
...
class GpioGPIOD(GpioDriverBase):
    name = "GPIOD(libgpiod)"
    def __init__(self, chip="/dev/gpiochip0", line=12):
        self.chip_path = chip
    ...
    def setup(self):
        ...
        self.line.request(consumer="aoa-server", type=gpiod.LINE_REQ_DIR_OUT, default_vals=[0])
    def pulse(self, count, on_ms, off_ms):
        if count <= 0: return
        count = min(count, MAX_PULSES)
        for _ in range(count):
...
class GpioSYSFS(GpioDriverBase):
    name = "SYSFS(/sys/class/gpio)"
    def __init__(self, gpio_n=12):
        self.n = gpio_n
        self.base = f"/sys/class/gpio/gpio{self.n}"
    def _write(self, path, data):
        with open(path, "w") as f:
            f.write(str(data))
    def setup(self):
        try:
            self._write("/sys/class/gpio/export", self.n)
        except Exception:
            pass
        time.sleep(0.05)
        self._write(f"{self.base}/direction", "out")
        self._write(f"{self.base}/value", "0")
    def pulse(self, count, on_ms, off_ms):
        if count <= 0: return
        count = min(count, MAX_PULSES)
        for _ in range(count):
            self._write(f"{self.base}/value", "1")
            time.sleep(on_ms/1000.0)
            self._write(f"{self.base}/value", "0")
            time.sleep(off_ms/1000.0)
    def cleanup(self):
        try:
            ...
            pass
        try:
            self._write("/sys/class/gpio/unexport", self.n)
        except Exception:
            pass

def select_gpio_backend(panel: Panel):
    for bk in GPIO_BACKEND_PRIORITY:
        try:
            if bk == "OPI":
                drv = GpioOPI(board=OPI_BOARD, pin=OPI_PIN)
                drv.setup()
                panel.add_event(f"GPIO backend: {drv.name} (pin BOARD {OPI_PIN})")
                return drv
            if bk == "RPI":
                drv = GpioRPI(bcm=RPI_BCM)
                drv.setup()
                panel.add_event(f"GPIO backend: {drv.name} (BCM {RPI_BCM})")
                return drv
            if bk == "GPIOD":
                drv = GpioGPIOD(chip=GPIOD_CHIP, line=GPIOD_LINE)
                drv.setup()
                panel.add_event(f"GPIO backend: {drv.name} ({GPIOD_CHIP}:{GPIOD_LINE})")
                return drv
            if bk == "SYSFS":
                drv = GpioSYSFS(gpio_n=SYSFS_GPIO_N)
                drv.setup()
                panel.add_event(f"GPIO backend: {drv.name} (gpio{SYSFS_GPIO_N})")
                return drv
        except Exception as e:
            panel.add_event(f"GPIO '{bk}' indisponível: {e}")
            continue
    panel.add_event("GPIO: nenhum backend disponível (pulsos desativados)")
            ...
            try:
                ...
                if data:
                    ...
                    while b'\n' in buffer:
                        ...
                        # 1.1) resposta local para o app
                                            # 1.1) resposta local para o app (DESATIVADA por padrão no Android 6)
                        ...
                        if SEND_BACK_TO_ANDROID:
                            try:
                                dev.write(ep_out.bEndpointAddress, confirm.encode('utf-8'), timeout=WRITE_TIMEOUT_MS)
                                session["tx_msgs"] += 1
                        ...
                        # 1.3) legado: GPIO local + acumulador
                        try:
...
def main_loop():
    panel = Panel()
    gpio = select_gpio_backend(panel)
